...
tbl_conv_plot <- tbl_conv %>%
  ...
  mutate(Conversion = as.numeric(gsub("%", "", Conversion)))

# Setting order
tbl_conv$Stage <- factor(tbl_conv$Stage, levels = stage_order)
...
ggplot(df, aes(x = score)) +
  geom_histogram(aes(y = ..count.., fill = ..count..), binwidth = 1, color = "white", boundary = 0, alpha = 0.9) +
...
This part examines whether candidates from certain application sources tend to achieve higher test scores than others.
...
Average scores are compared across job roles to identify any variation in candidate performance by function.
...
#### 3.2.6 Score by Location
Score distributions are explored by location to assess whether candidate quality varies regionally.
...
Compare the number of candidates managed, hires made, and overall hire rates by recruiter.
...
#### 3.3.2 Stage Conversion by Recruiter
Analyse how candidate progression differs by recruiter across funnel stages.


#### 3.3.3 Recruiter by Source


#### 3.3.4 Recruiter by loca

Examine how each recruitment channel performs in attracting and converting applicants.

3.10.1 Source Volume and Hire Rate

Compare the size and success rate of each applicant source.

# Table of applicants, hires, and hire rates by source

3.10.2 Stage Conversion by Source

Explore which sources drive the highest progression and conversion rates.

# Grouped bar chart of conversion by stage

3.10.3 Interpretation

Highlight the most effective sources and discuss trade-offs between quantity and quality.

### 3.11 Job Role Performance

Investigate how hiring outcomes differ across job functions.

3.11.1 Volume and Hire Rate by Role

Show which roles attract the most candidates and have the highest conversion rates.

# Applicants, hires, and conversion per role

3.11.2 Stage Conversion by Role

Assess at which stages attrition is highest by job function.

# Funnel comparison across roles

3.11.3 Interpretation

Comment on which roles appear most selective or challenging to fill.

### 3.12 Location Performance

Explore regional differences in process outcomes (not test scores, which you already covered).

3.12.1 Applicant and Hire Volume by Location

Summarise recruitment activity and success rates across regions.

# Applicants, hires, and hire rate by location

3.12.2 Stage Conversion by Location

Identify differences in stage progression by location.

# Conversion proportions by region and stage

3.12.3 Interpretation

Discuss potential reasons for geographic variation — local labour market, recruiter distribution, or role allocation.

### 4. Predictive Modelling

Combine all factors to quantify which most influence hiring outcomes and validate results through multiple models.

### 4.0 Model Comparison and Rationale

Brief explanation of using both logistic regression and random forest for interpretability and robustness.

#### 4.1 Multiple Logistic Regression
Estimate independent effects of all variables together.

# glm(hired ~ score + recruiter + source + role + location)

4.2 Decision Tree

Identify decision thresholds and interaction points.

# rpart / rpart.plot

4.3 Random Forest

Assess variable importance and model accuracy.

# randomForest + varImpPlot

4.4 Model Comparison and Synthesis
```{r}
